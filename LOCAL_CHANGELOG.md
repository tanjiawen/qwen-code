...
## v0.21.5-study.8 (2026-08-08)

**主题：版本簿记——对齐并行会话改动，补记 OOM 修复的版本归属**

### 背景

近期有两个会话在同时推进：远程已产生 study.2–study.7 六个版本（Better
Harness 状态面板、工程实践技能强化、OCR 评审强制化、gemini.tsx 修复等），
并合并了上游 v0.21.7（47 个提交）。而 dev 模式 OOM 修复
（performance-buffer-janitor）的 changelog 条目此前误用旧编号
v0.21.2-study.12，一直没有对应的真实版本号。

### 变更内容

- **版本对齐**：同步远程全部改动后，版本号递增至 0.21.5-study.8
- **changelog 补记**：为 janitor 修复条目添加编号说明——它在旧系列中原计划
  是 study.12，上游合并重置编号后随 0.21.5-study.x 系列发布，未单独占用
  版本号，本版本正式补记归属
...
> 编号说明：本修复原计划为旧系列 study.12，上游合并重置编号后随
> 0.21.5-study.x 系列发布，未单独占用版本号；详见 v0.21.5-study.8 条目。
